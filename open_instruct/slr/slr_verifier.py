...
def evaluate_prediction(prediction, validation_program, eval_config, timeout=5, isomorphic=True):
    """
    Evaluate a predicted Prolog rule against the validation program using SWI-Prolog.

    Constructs a Prolog program from the validation facts and the predicted rule,
    executes it via ``swipl``, and returns the fraction of examples covered correctly.

    Args:
        prediction: The predicted Prolog rule string.
        ...
        isomorphic: If True, normalise constant names before evaluation so that
            structurally equivalent rules with different constants are accepted.
