...
def slr_bench_prepare_v1(row: dict[str, Any], tokenizer: PreTrainedTokenizer, **kwargs: Any) -> dict[str, Any]:
    ...
    Converts the raw dataset columns (``prompt``, ``validation program``) into
    the standard RLVR format expected by the training pipeline: tokenised prompt,
    JSON-serialised ground truth, and verifier source tag.

    Rows that already contain a ``messages`` column are returned unchanged, which
    allows this transform to be safely composed with other datasets.
    """
