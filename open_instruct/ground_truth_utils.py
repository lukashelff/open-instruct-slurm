...
class SLRBenchVerifier(VerifierFunction):
    """
    Verifier for SLR-Bench (Scalable Logical Reasoning / Inductive Logic Programming) tasks.

    Evaluates a predicted Prolog rule against a symbolic validation program using
    SWI-Prolog (``swipl``). Both an isomorphic judge (which normalises constant
    names) and a base judge are run on every call; the score selected for
    training is controlled by ``SLRBenchVerifierConfig.slr_reward``. Both scores
    are returned in ``extra_scores`` for tracking.

    Label format: JSON-serialised dict with:
      - ``validation_program``: Prolog facts defining positive/negative examples.
    ...
    def __call__(
        ...
        """
        Score a predicted Prolog rule against the SLR-Bench validation program.

        Runs both the isomorphic and base Prolog judges. The score used for
        training is selected by ``verifier_config.slr_reward``; both scores
        are returned in ``extra_scores`` for logging.
        """
        if not prediction:
        ...
        if reward_model not in ["isomorphic", "base"]:
            logger.warning(
                "[SLRBenchVerifier] Invalid slr_reward config '%s', defaulting to 'isomorphic'."
                " Must be 'isomorphic' or 'base'.",
                reward_model,
            )
