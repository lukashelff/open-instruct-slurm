...
#SBATCH --job-name=RLVR-soofi-Basev2-Isomorphic-RL
...
JOB_NAME="RLVR-soofi-Basev2-Isomorphic-RL"
...
GRPO_ARGS="--exp_name $JOB_NAME \
  ...
  --dataset_local_cache_dir /stage/.cache/open_instruct_dataset_cache2 \
  --kl_estimator 2 \
...
srun --nodes=3 --ntasks=3 apptainer exec --nv "${APPTAINER_ENV[@]}" "$CONTAINER_IMAGE" \
  bash -c '
    cd /stage
    mkdir -p /tmp/triton
